...
void __ISR(_EXTERNAL_0_VECTOR, IPL2SOFT) Ext0ISR(void) { // step 1: the ISR
  ...
  //if USER button is not pressed after the ISR is triggered, then ISR was 
  //triggered upon release of button, so do nothing
  if (NU32_USER) {
	  return;
  }
  ...
  while(_CP0_GET_COUNT() < 10000000) { ; } // delay for 10M core ticks, 0.25s
