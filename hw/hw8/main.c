...
void __ISR(_TIMER_3_VECTOR, IPL5SRS) CurrentControl(void) {
	...
	switch (mode) {
		...
		case IDLE:
		{
			...
			//in phase/enable mode: send a 0 to enable bit to brake
			LATDbits.LATD5 = 0;
...
void initialize_sfrs(void) {
	...
	//digital output for setting enable pin for H-bridge braking?
	//wasn't in the instructions but I wanted to add it
	TRISDbits.TRISD5 = 0;
	LATDbits.LATD5 = 1;
